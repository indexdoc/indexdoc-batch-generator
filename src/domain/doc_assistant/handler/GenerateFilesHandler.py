from domain.doc_assistant.serv import GenerateFilesServ
...
class ApiVsListHandler(BaseApiHandler):
    def myget(self):
        ...
        _entities,_cnt = GenerateFilesServ.get_vspage(_limit, (_page - 1) * _limit, _search_params)
        self.write({
            'success':True,
            'code':0,
            'msg':None,
...
class ApiVsFullSearchHandler(BaseApiHandler):
    def myget(self):
        ...
        if _search_str is None or _search_str.strip() == '':
            ...
            return
        _entities,_cnt = GenerateFilesServ.full_search(_limit, (_page - 1) * _limit, _search_str)
        self.write({
            'success':True,
            'code':0,
            'msg':None,
...
class ApiListHandler(BaseApiHandler):
    def myget(self):
        ...
        if _search_params is not None:
            _search_params = json.loads(_search_params)
        _entities,_cnt = GenerateFilesServ.get_page(_limit, (_page - 1) * _limit, _search_params)
        self.write({
            'success':True,
            'code':0,
            'msg':None,
...
class ApiAddHandler(BaseApiHandler):
    def mypost(self):
        ...
        _entity = GenerateFilesServ.create(_user_file_id,_excel_template_id,_create_time)
        self.clear()
        _rtn = {'success': True,
                'msg': 'success',
                'obj': _entity.to_dict(),
...
class ApiDeleteHandler(BaseApiHandler):
    def myget(self):
        _generate_file_id = int(self.get_arg('generate_file_id'))
        GenerateFilesServ.delete(_generate_file_id)
        _rtn = {'success': True,
                'msg': 'success',
                'obj': _generate_file_id,
                }
        self.write(_rtn)
        return
...
class ApiUpdateHandler(BaseApiHandler):
    def mypost(self):
        _generate_file_id=str_to_int(self.get_arg('generate_file_id'))
        _user_file_id=str_to_int(self.get_arg('user_file_id'))
        _excel_template_id=str_to_int(self.get_arg('excel_template_id'))
        _create_time=str_to_datetime(self.get_arg('create_time'))
        _entity = GenerateFilesServ.update(_generate_file_id ,_user_file_id ,_excel_template_id ,_create_time )
        _rtn = {'success': True,
                'msg': 'success',
                'obj': _entity.to_dict(),
                }
...
# /api/doc_assistant/GenerateFiles/updateById?id=123456&update_params={"column_code1":"value1","column_code2":"value2"}
class ApiUpdateByIdHandler(BaseApiHandler):
    def myget(self):
        _generate_file_id = self.get_arg('id')
        _update_params = self.get_arg('update_params') #JSON格式传数据
        if _generate_file_id is None or _generate_file_id.strip() == '' \
...
#将参数转换为python对象
        if _update_params.get('generate_file_id') is not None:
            _update_params['generate_file_id'] = str_to_int(_update_params.get('generate_file_id'))
        ...
        _entity = GenerateFilesServ.update_by_id(_generate_file_id,_update_params)
        if _entity is None:
            self.write({
                'success':False,
                'msg':'ID不存在',
...
urls =[
    ('/api/doc_assistant/GenerateFiles/list',ApiListHandler),
    ('/api/doc_assistant/GenerateFiles/vslist',ApiVsListHandler),
    ('/api/doc_assistant/GenerateFiles/add', ApiAddHandler),
    ('/api/doc_assistant/GenerateFiles/delete', ApiDeleteHandler),
    ('/api/doc_assistant/GenerateFiles/update', ApiUpdateHandler),
    ('/api/doc_assistant/GenerateFiles/fullSearch', ApiVsFullSearchHandler),
    ('/api/doc_assistant/GenerateFiles/updateById', ApiUpdateByIdHandler),
    ]
