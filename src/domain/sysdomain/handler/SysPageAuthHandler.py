from domain.sysdomain.serv import SysPageAuthServ
...
class ApiVsListHandler(BaseApiHandler):
    def myget(self):
        ...
        _entities,_cnt = SysPageAuthServ.get_vspage(_limit, (_page - 1) * _limit, _search_params)
        self.write({
            'success':True,
            'code':0,
            'msg':None,
...
class ApiVsFullSearchHandler(BaseApiHandler):
    def myget(self):
        ...
        if _search_str is None or _search_str.strip() == '':
            ...
            return
        _entities,_cnt = SysPageAuthServ.full_search(_limit, (_page - 1) * _limit, _search_str)
        self.write({
            'success':True,
            'code':0,
            'msg':None,
...
class ApiListHandler(BaseApiHandler):
    def myget(self):
        ...
        if _search_params is not None:
            _search_params = json.loads(_search_params)
        _entities,_cnt = SysPageAuthServ.get_page(_limit, (_page - 1) * _limit, _search_params)
        self.write({
            'success':True,
            'code':0,
            'msg':None,
...
class ApiAddHandler(BaseApiHandler):
    def mypost(self):
        _page_name=object_to_str(self.get_arg('page_name'))
        _page_path=object_to_str(self.get_arg('page_path'))
        ...
        _entity = SysPageAuthServ.create(_page_name,_page_path,_sys_user_id,_sys_role_id,_sys_org_id,_org_duty_id,_auth_flag,_sys_menu_auth_id,_update_type,_remark)
        self.clear()
        _rtn = {'success': True,
                'msg': 'success',
                'obj': _entity.to_dict(),
...
class ApiDeleteHandler(BaseApiHandler):
    def myget(self):
        _sys_page_auth_id = int(self.get_arg('sys_page_auth_id'))
        SysPageAuthServ.delete(_sys_page_auth_id)
        _rtn = {'success': True,
                'msg': 'success',
                'obj': _sys_page_auth_id,
                }
        self.write(_rtn)
        return
...
class ApiUpdateHandler(BaseApiHandler):
    def mypost(self):
        _sys_page_auth_id=str_to_int(self.get_arg('sys_page_auth_id'))
        _page_name=object_to_str(self.get_arg('page_name'))
        _page_path=object_to_str(self.get_arg('page_path'))
        _sys_user_id=str_to_int(self.get_arg('sys_user_id'))
        _sys_role_id=str_to_int(self.get_arg('sys_role_id'))
        ...
        _update_type=object_to_str(self.get_arg('update_type'))
        _remark=object_to_str(self.get_arg('remark'))
        _entity = SysPageAuthServ.update(_sys_page_auth_id ,_page_name ,_page_path ,_sys_user_id ,_sys_role_id ,_sys_org_id ,_org_duty_id ,_auth_flag ,_sys_menu_auth_id ,_update_type ,_remark )
        _rtn = {'success': True,
                'msg': 'success',
                'obj': _entity.to_dict(),
                }
...
# /api/sysdomain/SysPageAuth/updateById?id=123456&update_params={"column_code1":"value1","column_code2":"value2"}
class ApiUpdateByIdHandler(BaseApiHandler):
    def myget(self):
        _sys_page_auth_id = self.get_arg('id')
        _update_params = self.get_arg('update_params') #JSON格式传数据
        if _sys_page_auth_id is None or _sys_page_auth_id.strip() == '' \
...
#将参数转换为python对象
        if _update_params.get('sys_page_auth_id') is not None:
            _update_params['sys_page_auth_id'] = str_to_int(_update_params.get('sys_page_auth_id'))
        if _update_params.get('page_name') is not None:
            _update_params['page_name'] = object_to_str(_update_params.get('page_name'))
        if _update_params.get('page_path') is not None:
            _update_params['page_path'] = object_to_str(_update_params.get('page_path'))
        ...
        _entity = SysPageAuthServ.update_by_id(_sys_page_auth_id,_update_params)
        if _entity is None:
            self.write({
                'success':False,
                'msg':'ID不存在',
...
urls =[
    ('/api/sysdomain/SysPageAuth/list',ApiListHandler),
    ('/api/sysdomain/SysPageAuth/vslist',ApiVsListHandler),
    ('/api/sysdomain/SysPageAuth/add', ApiAddHandler),
    ('/api/sysdomain/SysPageAuth/delete', ApiDeleteHandler),
    ('/api/sysdomain/SysPageAuth/update', ApiUpdateHandler),
    ('/api/sysdomain/SysPageAuth/fullSearch', ApiVsFullSearchHandler),
    ('/api/sysdomain/SysPageAuth/updateById', ApiUpdateByIdHandler),
    ]
