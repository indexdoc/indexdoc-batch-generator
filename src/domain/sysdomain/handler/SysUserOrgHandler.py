from domain.sysdomain.serv import SysUserOrgServ
...
class ApiVsListHandler(BaseApiHandler):
    def myget(self):
        ...
        _entities,_cnt = SysUserOrgServ.get_vspage(_limit, (_page - 1) * _limit, _search_params)
        self.write({
            'success':True,
            'code':0,
            'msg':None,
...
class ApiVsFullSearchHandler(BaseApiHandler):
    def myget(self):
        ...
        if _search_str is None or _search_str.strip() == '':
            ...
            return
        _entities,_cnt = SysUserOrgServ.full_search(_limit, (_page - 1) * _limit, _search_str)
        self.write({
            'success':True,
            'code':0,
            'msg':None,
...
class ApiListHandler(BaseApiHandler):
    def myget(self):
        ...
        if _search_params is not None:
            _search_params = json.loads(_search_params)
        _entities,_cnt = SysUserOrgServ.get_page(_limit, (_page - 1) * _limit, _search_params)
        self.write({
            'success':True,
            'code':0,
            'msg':None,
...
class ApiAddHandler(BaseApiHandler):
    def mypost(self):
        ...
        _entity = SysUserOrgServ.create(_sys_user_id,_sys_org_id,_org_duty_id,_update_time,_create_time)
        self.clear()
        _rtn = {'success': True,
                'msg': 'success',
                'obj': _entity.to_dict(),
...
class ApiDeleteHandler(BaseApiHandler):
    def myget(self):
        _sys_user_org_id = int(self.get_arg('sys_user_org_id'))
        SysUserOrgServ.delete(_sys_user_org_id)
        _rtn = {'success': True,
                'msg': 'success',
                'obj': _sys_user_org_id,
                }
        self.write(_rtn)
        return
...
class ApiUpdateHandler(BaseApiHandler):
    def mypost(self):
        _sys_user_org_id=str_to_int(self.get_arg('sys_user_org_id'))
        _sys_user_id=str_to_int(self.get_arg('sys_user_id'))
        _sys_org_id=str_to_int(self.get_arg('sys_org_id'))
        _org_duty_id=str_to_int(self.get_arg('org_duty_id'))
        _update_time=str_to_datetime(self.get_arg('update_time'))
        _create_time=str_to_datetime(self.get_arg('create_time'))
        _entity = SysUserOrgServ.update(_sys_user_org_id ,_sys_user_id ,_sys_org_id ,_org_duty_id ,_update_time ,_create_time )
        _rtn = {'success': True,
                'msg': 'success',
                'obj': _entity.to_dict(),
                }
...
# /api/sysdomain/SysUserOrg/updateById?id=123456&update_params={"column_code1":"value1","column_code2":"value2"}
class ApiUpdateByIdHandler(BaseApiHandler):
    def myget(self):
        _sys_user_org_id = self.get_arg('id')
        _update_params = self.get_arg('update_params') #JSON格式传数据
        if _sys_user_org_id is None or _sys_user_org_id.strip() == '' \
...
#将参数转换为python对象
        if _update_params.get('sys_user_org_id') is not None:
            _update_params['sys_user_org_id'] = str_to_int(_update_params.get('sys_user_org_id'))
        ...
        _entity = SysUserOrgServ.update_by_id(_sys_user_org_id,_update_params)
        if _entity is None:
            self.write({
                'success':False,
                'msg':'ID不存在',
...
urls =[
    ('/api/sysdomain/SysUserOrg/list',ApiListHandler),
    ('/api/sysdomain/SysUserOrg/vslist',ApiVsListHandler),
    ('/api/sysdomain/SysUserOrg/add', ApiAddHandler),
    ('/api/sysdomain/SysUserOrg/delete', ApiDeleteHandler),
    ('/api/sysdomain/SysUserOrg/update', ApiUpdateHandler),
    ('/api/sysdomain/SysUserOrg/fullSearch', ApiVsFullSearchHandler),
    ('/api/sysdomain/SysUserOrg/updateById', ApiUpdateByIdHandler),
    ]
