...
def select_count():
    sql = "select count(*) from view_user_menu"
    rts = exesql(sql)
    return int(rts[0][0])

def select_all(order_by = ""):
    sql = "select sys_user_id, sys_menu_id, upper_id, title, order_no, href, icon, target, remark from view_user_menu" + order_by
    rts = exesql(sql)
    rts_list = []
    for tup in rts:
        _dic = {'sys_user_id':tup[0], 'sys_menu_id':tup[1], 'upper_id':tup[2], 'title':tup[3], 'order_no':tup[4], 'href':tup[5], 'icon':tup[6], 'target':tup[7], 'remark':tup[8]}
...
def full_search(search_str, row_cnt = 50000, row_begin=0, order_by = ""):
    ...
    attrs = ['sys_user_id','sys_menu_id','upper_id','title','order_no','href','icon','target','remark']
    for attr in attrs:
        _where += " toString(%s) like '%%%s%%' or"%(attr,search_str)
    _where = _where + ' 1!=1'
    attrstr = 'sys_user_id, sys_menu_id, upper_id, title, order_no, href, icon, target, remark'
    sql = "select %s from view_user_menu "%attrstr + _where + \
          order_by + _limit
...
def select_page(row_cnt, row_begin, search_params,order_by = ""):
    ...
    _where = _where + ' 1=1'
    sql = "select sys_user_id, sys_menu_id, upper_id, title, order_no, href, icon, target, remark from view_user_menu " + _where + \
