from domain.sysdomain.view import ViewUserApiDaoCK
...
class ApiListHandler(BaseApiHandler):
    def myget(self):
        ...
        _dict_list,_cnt = ViewUserApiDaoCK.select_page(int(_limit), (int(_page) - 1) * int(_limit),_search_params)
        self.write({
            'success':True,
            'code':0,
            'msg':None,
...
class ApiFullSearchHandler(BaseApiHandler):
    def myget(self):
        ...
        if _search_str is None or _search_str.strip() == '':
            ...
            return
        _dict_list,_cnt = ViewUserApiDaoCK.full_search(_search_str, _limit, (_page - 1) * _limit)
        self.write({
            'success':True,
            'code':0,
            'msg':None,
...
urls =[
    ('/api/sysdomain/ViewUserApi/list',ApiListHandler),
    ('/api/sysdomain/ViewUserApi/fullSearch', ApiFullSearchHandler),
    ]
