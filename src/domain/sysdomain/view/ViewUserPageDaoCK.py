...
def select_count():
    sql = "select count(*) from view_user_page"
    rts = exesql(sql)
    return int(rts[0][0])

def select_all(order_by = ""):
    sql = "select sys_user_id, sys_page_auth_id, page_path, auth_flag, update_type from view_user_page" + order_by
    rts = exesql(sql)
    rts_list = []
    for tup in rts:
        _dic = {'sys_user_id':tup[0], 'sys_page_auth_id':tup[1], 'page_path':tup[2], 'auth_flag':tup[3], 'update_type':tup[4]}
...
def full_search(search_str, row_cnt = 50000, row_begin=0, order_by = ""):
    ...
    attrs = ['sys_user_id','sys_page_auth_id','page_path','auth_flag','update_type']
    for attr in attrs:
        _where += " toString(%s) like '%%%s%%' or"%(attr,search_str)
    _where = _where + ' 1!=1'
    attrstr = 'sys_user_id, sys_page_auth_id, page_path, auth_flag, update_type'
    sql = "select %s from view_user_page "%attrstr + _where + \
          order_by + _limit
    sql_count = "select count(*) from view_user_page " + _where
    _dict_list = []
    rts = exesql(sql)
...
def select_page(row_cnt, row_begin, search_params,order_by = ""):
    ...
    _where = _where + ' 1=1'
    sql = "select sys_user_id, sys_page_auth_id, page_path, auth_flag, update_type from view_user_page " + _where + \
